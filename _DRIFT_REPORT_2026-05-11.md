...
## 11. Post-2026-05-11 reversals

_This section logs deliberated reversals of decisions that landed implicitly in the May 11 specs reset. Each entry names what was lost, why, and how it returns. Format mirrors §3 conflicts._

### Conflict 21 — Per-goal pricing (May 11 implicit drop; 2026-05-15 reversal)

**Existing canon before May 11** (V3.1 build spec at `aspire-gtm/20_PRODUCT_ENGINE/SITE_V3.1_REVIEW/07_V3.1_BUILD_SPEC_PHASE_1.md`, dated 2026-05-06): the Calculator collected priced goals via a 10-button goal-type picker. Each goal row had columns `What | When | Today's price | Cost grows like | (remove)`. The Aspire Rate computed against a real dollar shortage.

**What the May 11 specs did:** replaced the priced-goal Calculator with a qualitative Home / Family / Freedom chip picker. No per-goal price input. The Calculator's `goals` array was generated by splitting the user's `totalAssets` across basket-component weights — i.e. goal "price" became a function of *what the user already had*, not *what they wanted*. This was an implicit consequence of choosing a faster Calculator path, not a deliberated decision logged in this drift report at the time.

**Why this was wrong** (Scott's framing, 2026-05-15): *"There isn't a lot of pain without a dollar shortage."* The Aspire Rate alone is math; the rate paired with a priced goal is the felt cost. A $75k starting balance cannot reach a $1.5M Freedom number at any rate inside any timeline — that is the read a rate-only product hides. The May 11 reset was a speed decision the team now reverses.

**How pricing returns (resolved 2026-05-15, Option 2):**

- **Calculator** — adds a single editable dollar field beside the existing Home / Family / Freedom chip picker, pre-filled with a Hermes-grounded median. One goal, one price, one horizon. Smallest possible diff from current live; preserves the May 11 onboarding speed.
- **Simulator** — adds a Goals lever (per `aspire/specs/page-spec-simulator-amendment-goals-lever-v3.md`) where users append, edit, and remove additional priced goals after the email gate. The Calculator's priced goal carries through as the first row.
- **Helper copy** at the Calculator goal row: *"One goal sets the headline. You'll add the rest in the Simulator."* CTA supporting line: *"Open the Simulator to add goals, tune assumptions, and run scenarios."* Both pass `npm run check:compliance`.

**What V3.1 contributed even though it was not shipped:**
- The conceptual asymmetry between `costInflator` (goals) and `fundingVehicle` (holdings) in `rates.json` filtering. Carried forward.
- The taxonomy of goal types (Home, College, Retirement, Wedding, Car, Children, Time off, Business, …). Deferred to a later phase; Option 2 ships with a single basket-keyed goal.
- The hero copy: *"Your future has its own inflation rate. Measure the gap."* Recommend revisiting in a later voice pass; not in scope for the 2026-05-15 reversal.

**Seed values** (resolved 2026-05-15, sourced by Hermes in `aspire-gtm/70_AGENT_LAB/threads/2026-05-15-goals-seed-defaults-grounding.md`):
- Home: **$420,000** — NAR median existing-home sale price, April 2026.
- Family — raise a child to 18: **$330,000** — USDA 2015 baseline ($233,610) × CPI-U 2015→Apr 2026 inflation.
- Freedom: **$2,000,000** — 25× BLS Consumer Expenditure Survey 2024 average annual expenditures ($78,535) per the 4% withdrawal-rate heuristic (Bengen 1994 / Trinity 1998).

Refresh cadence: NAR monthly, CPI monthly (Family seed recomputed automatically), BLS CE annual. Seeded values are user-editable; on first edit, the source label drops and the row converts to user-owned.

**Status:** Reversed. Spec amendment v3 in flight. Codex implementation will branch `codex/simulator-goals-lever` once the spec is promoted.

**What this reversal does not change:**
- Aspire Rate / Aspire Gap math signatures in `lib/aspire.js`.
