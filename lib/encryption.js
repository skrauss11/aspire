...
async function getEncryptionKey() {
  ...
  const envKey = process.env.ASPIRE_FIELD_ENCRYPTION_KEY;
  if (envKey) {
    try {
      cachedKey = normalizeKey(envKey);
      return cachedKey;
    } catch (err) {
      console.warn("ASPIRE_FIELD_ENCRYPTION_KEY is not usable; falling back to Supabase Vault:", err.message);
    }
  }
  const raw = await fetchKeyFromVault(VAULT_SECRET_NAME);
