import { futureAssetValue, futureGoalCost, project } from "../aspire.js";
...
function goalShort(goals, holdings) {
  const priced = goals
    .filter(goal => Number(goal.amount) > 0 && Number(goal.horizon) > 0)
    .map(goal => {
      const futureCost = futureGoalCost(goal, goal.horizon);
      const projected = holdings.reduce((sum, asset) => sum + futureAssetValue(asset, goal.horizon), 0);
      return {
        goal,
        futureCost,
        projected,
        coverage: futureCost ? projected / futureCost : 0,
        shortfall: Math.max(0, futureCost - projected)
      };
    })
    .filter(item => item.futureCost > 0 && item.projected < item.futureCost)
    .sort((a, b) => b.shortfall - a.shortfall);
  if (!priced.length) return null;
  const item = priced[0];
  const coverage = Math.max(0, Math.round(item.coverage * 100));
  const label = cleanLabel(item.goal.label || item.goal.goalType || "this goal");
  return {
    id: "goal-short",
    score: item.shortfall / Math.max(1, item.futureCost) * 140,
    observation: `At your target year, your projected portfolio covers ${coverage}% of ${label} at these assumptions. The shortfall is ${shortMoney(item.shortfall)}.`,
    pointer: null
  };
}
