...
function normalizeRate(rate = {}) {
  const trailing = Number(rate.cagrTrailing ?? rate.cagr) || 0;
  const forward = Number(rate.cagrForwardSuggested ?? trailing) || 0;
  return {
    ...rate,
    cagrTrailing: trailing,
    cagrForwardSuggested: forward
  };
}
